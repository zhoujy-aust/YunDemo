...
void MyFileWg::initListWidget()
{
    ...
    //上传下载信号和槽函数连接
//    connect(this, &MyFileWg::uploadsignal, this, &MyFileWg::uploadFilesAction);
//    connect(this, &MyFileWg::downloadsignal, this, &MyFileWg::downloadFilesAction);
}
...
void MyFileWg::uploadFilesAction()
{
    ...
    QString url = QString("http://%1:%2/md5").arg( login->getIp(),login->getPort() );
...
{
//    cout << "uploadFile被调用，调用线程ID为：" << QThread::currentThread();
    //取出上传任务
    ...
    */
    ...
    //第1行，分隔线
    data.append(boundary.toUtf8().data());
    data.append("\r\n");

    //上传文件信息
    data.append("Content-Disposition: form-data; ");
    data.append( QString("user=\"%1\" ").arg( login->getUser() ).toUtf8().data() ); //上传用户
    data.append( QString("filename=\"%1\" ").arg(fileName).toUtf8().data()); //文件名字
    data.append( QString("md5=\"%1\" ").arg(md5).toUtf8().data()); //文件md5码
    data.append( QString("size=%1").arg(size).toUtf8().data());   //文件大小
    data.append("\r\n");

    data.append("Content-Type: application/octet-stream"); //二进制流，不明文件类型
    data.append("\r\n");
    data.append("\r\n");
    ...
    QString url = QString("http://%1:%2/upload").arg(login->getIp(), login->getPort());
    request.setUrl(QUrl( url )); //设置url
...
void MyFileWg::refreshFiles(MyFileWg::Display cmd)
{
    ...
    QString url = QString("http://%1:%2/myfiles?cmd=count").arg(login->getIp(), login->getPort());
    request.setUrl(QUrl( url )); //设置url
...
void MyFileWg::downloadFilesAction()
{
    ...
    connect(reply, &QNetworkReply::readyRead, [=]()
    {
        ...
        if (file != NULL)
        {
            file->write(reply->readAll());//文件保存到本地
        }
    });
