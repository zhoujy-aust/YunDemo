...
//     QDesktopWidget* desktop = QApplication::desktop();
     ...
     // 移动窗口
     tmp->move((desktop->geometry().width() - tmp->width())/2, (desktop->geometry().height() - tmp->height())/2);
}
...
QString Common::getBoundary()
{
    ...
    for(int i = 0; i < 16; i++)
    {
        tmp.append((char)(QRandomGenerator::global()->bounded(48, 123))); //随机48-123之间的整数， 不包括123
    }
...
{
//    cout << "getNetManager被调用，调用线程ID为：" << QThread::currentThread();
